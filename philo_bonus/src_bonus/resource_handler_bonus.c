...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/05 19:10:20 by yaktas            #+#    #+#             */
/*   Updated: 2022/12/05 19:10:20 by yaktas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	eat_spagetti(t_philosopher *philo)
{
	...
	action_philo(rules, philo->id, "has taken a fork");
	sem_wait(rules->forks);
	action_philo(rules, philo->id, "has taken a fork");
	sem_wait(rules->meal_check);
	action_philo(rules, philo->id, "is eating");
...
void	resources(void *void_philosopher)
{
	...
	while (!(rules->died))
	{
		if (philo->ate_count >= rules->number_must_eat
			...
			break ;
		eat_spagetti(philo);
		action_philo(rules, philo->id, "is sleeping");
		wsleep(rules->time_to_sleep, rules);
		action_philo(rules, philo->id, "is tkinking");
	}
